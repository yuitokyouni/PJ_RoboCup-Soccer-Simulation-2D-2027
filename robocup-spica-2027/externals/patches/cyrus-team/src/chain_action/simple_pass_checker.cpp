                               ...
                               const double & first_ball_speed ) const
{
    ...
    // PHASE9D_BACKPASS: never recommend pass-to-our-GK from any
    // outfield teammate. The rcssserver back-pass rule triggers an
    // indirect FK INSIDE our penalty area when the GK catches a
    // teammate's intentional kicked pass. Phase 9c match analysis
    // (notes/2026-06-27_rev_improvement_points.md, Goal G2 at cycle
    // 2905) had exactly this sequence: outfield -> GK -> back_pass
    // call -> indirect FK in PA -> goal. The existing guards above
    // only reject the pass when the receive point is INSIDE the PA,
    // but a kicked pass anywhere can reach the GK. Reject all
    // outfield-to-GK pass candidates as the simplest fix.
    if ( to.goalie() && ! from.goalie() ) {
#ifdef DEBUG_PRINT
        dlog.addText( Logger::PASS,
                      "(SimplePassChecker) %d to %d: outfield->GK pass rejected (Phase 9d back-pass guard)",
                      from.unum(), to.unum() );
#endif
        return false;
    }
